...
bool NetworkCore::InitNetworkCore( funcOnAccept_t onAccpet, funcOnClose_t onClose )
{
	try
	{
		...
		networkImpl_->networkModel_ = std::make_shared<AsioModel>();
	}
	catch( std::bad_alloc& )
	{
		return false;
...
Session* NetworkCore::CreateSession( boost::asio::io_service& io_service )
{
	return networkImpl_->sessionManager_->CreateSession( io_service );
}
